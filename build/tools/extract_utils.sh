...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$AOSP_ROOT"/prebuilts/tools-aosp/common/smali/baksmali.jar
        export SMALIJAR="$AOSP_ROOT"/prebuilts/tools-aosp/common/smali/smali.jar
    fi

    if [ -z "$VDEXEXTRACTOR" ]; then
        export VDEXEXTRACTOR="$AOSP_ROOT"/prebuilts/tools-aosp/"${HOST,,}"-x86/bin/vdexExtractor
    fi

    if [ -z "$CDEXCONVERTER" ]; then
        export CDEXCONVERTER="$AOSP_ROOT"/prebuilts/tools-aosp/"${HOST,,}"-x86/bin/compact_dex_converter
    fi

    if [ -z "$PATCHELF" ]; then
        export PATCHELF="$AOSP_ROOT"/prebuilts/tools-lineage/${HOST}-x86/bin/patchelf
    fi
