...
#Extended Versioning
ANDRDOID_VERSION = 7.1.2
EXTENDED_VERSION = v4.0

    EXTENDED_BUILD_TYPE_BETA := BETA
    PLATFORM_VERSION_CODENAME := BETA

ifeq ($(EXTENDED_BUILD_TYPE_BETA), OFFICIAL)
PRODUCT_PACKAGES += \
	AEXOTA
endif

EXTENDED_MOD_VERSION := AospExtended-$(EXTENDED_VERSION)-$(shell date -u +%Y%m%d-%H%M)-$(EXTENDED_BUILD_TYPE_BETA)

PRODUCT_PROPERTY_OVERRIDES += \
  ro.extended.version=$(EXTENDED_VERSION) \
  ro.extended.releasetype=$(EXTENDED_BUILD_TYPE_BETA) \
  ro.modversion=$(EXTENDED_MOD_VERSION)
  
EXTENDED_DISPLAY_VERSION := AospExtended-$(EXTENDED_VERSION)-$(EXTENDED_BUILD_TYPE_BETA)
